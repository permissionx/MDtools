...
def find_neibour(self, atoms):
	atoms_sorted = sorted(atoms.items(), key=lambda item:c_distance(item[1].r,self.r))
...
def sia_dirction(self):
	...
	if self.properties['c_2[1]'] == 0: self.dirction=[0, 0, 0]
...
steps = [step]
mt.wdump(steps, 'cell_direction.dump', ['id','type','x','y','z','c_2[1]','d1','d2','d3'])